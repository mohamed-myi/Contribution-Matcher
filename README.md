...
For detailed command reference, see [docs/COMMAND_REFERENCE.md](docs/COMMAND_REFERENCE.md).
...
├── contribution_matcher.db   # SQLite database (tracked in git)
...
- **[Command Reference](docs/COMMAND_REFERENCE.md)** - Complete command documentation with examples
- **[GitHub Actions Setup](docs/GITHUB_ACTIONS_SETUP.md)** - Automated daily discovery setup

## Notes

- `dev_profile.json` and ML model files (`issue_classifier.pkl`, `issue_scaler.pkl`) are excluded from git for privacy
- Database (`contribution_matcher.db`) is tracked in git for GitHub Actions persistence
