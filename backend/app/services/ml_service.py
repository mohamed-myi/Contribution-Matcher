...
def label_status(db: Session, user: User) -> Dict[str, int]:
    ...
    remaining = (
        ...
        .filter(~exists().where((IssueLabel.issue_id == Issue.id) & (IssueLabel.user_id == user.id)))
        .count()
    )
    return {"total": total, "good": good, "bad": bad, "remaining": remaining}


def unlabeled_issues(db: Session, user: User, limit: int = 20, include_others: bool = False) -> List[Issue]:
    """
    ...
    subquery = (
        ...
        .filter(IssueLabel.user_id == user.id)
        .subquery()
    )
...
def labeled_issues(
    ...
    results = (
        base_query
        .order_by(IssueLabel.labeled_at.desc())
        .offset(offset)
        .limit(limit)
        .all()
    )
...
def _build_training_dataset(db: Session, user: User) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if not labeled:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="No labeled issues found.")
...
def model_info(db: Session, user: User) -> Optional[ModelInfoResponse]:
    record = (
        db.query(UserMLModel)
