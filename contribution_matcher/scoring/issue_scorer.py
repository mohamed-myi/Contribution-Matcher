...
from contribution_matcher.config import (
...
from contribution_matcher.database import get_issue_technologies, query_issues
from contribution_matcher.profile import load_dev_profile
from contribution_matcher.scoring.ml_trainer import predict_issue_quality
...
def calculate_skill_match(user_skills: List[str], tech_stack: List[str]) -> Tuple[float, List[str], List[str]]:
    ...
    # Lowercase for proper comparison
    profile_skills_lower = [s.lower() for s in user_skills]
    issue_techs_lower = [t.lower() for t in tech_stack]
    ...
    for issue_tech in tech_stack:
        issue_tech_lower = issue_tech.lower()
        # Check for exact match or substring match
        matched = False
        for profile_skill in profile_skills_lower:
            if issue_tech_lower == profile_skill or issue_tech_lower in profile_skill or profile_skill in issue_tech_lower:
...
def score_issue_against_profile(profile: Dict, issue_data: Dict) -> Dict:
    ...
    # Apply ML adjustment (30% weight on ML, 70% on rule-based)
    ml_weight = 0.3
    rule_weight = 0.7
