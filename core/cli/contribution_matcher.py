...
from core.api import get_repo_metadata_from_api, search_issues, batch_get_repo_metadata, batch_check_issue_status
...
from core.parsing import analyze_job_text
...
def cmd_discover_async(args):
    ...
    try:
        from workers.tasks import discover_issues_task, batch_discover_task
...
def cmd_list(args):
    ...
    if output_format and output_format != 'text':
        try:
            output = format_output(issues, output_format, verbose=verbose, output_file=getattr(args, 'output', None))
            if output:
                print(output)
                return
        except Exception as e:
...
def _generate_recommendations(breakdown: Dict, issue_data: Dict) -> List[str]:
    ...
    if time_score < 5:
        recommendations.append("Time estimate may exceed your availability - consider smaller issues")
...
def cmd_create_profile(args):
    ...
    if args.github:
        try:
            profile = create_profile_from_github(args.github)
        except Exception as e:
        ...
        try:
            profile = create_profile_manual(profile_data)
        except Exception as e:
...
def cmd_export(args):
    ...
    if args.format.lower() in ['csv', 'excel']:
        format_output(issues, args.format.lower(), verbose=getattr(args, 'verbose', False), output_file=args.output)
...
def main():
    ...
    discover_parser.add_argument('--no-quality-filters', action='store_true', help='Disable quality filters')
    discover_parser.add_argument('--verbose', '-v', action='store_true', help='Verbose output')
    
    # Async discover command
    discover_async_parser = subparsers.add_parser('discover-async', help='Discover issues asynchronously (Celery)')
    ...
    discover_async_parser.add_argument('--batch', action='store_true', help='Run multiple strategies')
    ...
    list_parser.add_argument('--format', choices=['text', 'json', 'table', 'csv', 'excel', 'markdown', 'html'], default='text')
    ...
    # Score async command
    score_async_parser = subparsers.add_parser('score-async', help='Score issues asynchronously (Celery)')
    ...
    variety_parser = subparsers.add_parser('variety-stats', help='Show variety statistics')
    ...
    # Cache commands
    cache_status_parser = subparsers.add_parser('cache-status', help='Show cache status')
    ...
    export_parser.add_argument('--format', choices=['csv', 'json', 'excel', 'markdown', 'html'], default='csv')
