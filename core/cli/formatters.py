...
def format_text(issues: List[Dict], verbose: bool = False) -> str:
    ...
    for i, issue in enumerate(issues, 1):
        ...
        if verbose:
            output.append(f"   Repo: {issue.get('repo_owner', 'N/A')}/{issue.get('repo_name', 'N/A')}")
...
def format_table(issues: List[Dict], verbose: bool = False) -> str:
    ...
    for i, issue in enumerate(issues, 1):
        ...
        if verbose:
            ...
            widths['Difficulty'] = max(widths.get('Difficulty', 0), len(str(issue.get('difficulty', ''))))
        if issue.get('score'):
...
def format_csv(issues: List[Dict], output_file: Optional[str] = None) -> Optional[str]:
    '''
    ...
    fieldnames = ['title', 'url', 'repo_owner', 'repo_name', 'issue_type', 
                  'difficulty', 'time_estimate', 'score']
...
def format_excel(issues: List[Dict], output_file: str) -> None:
    ...
    # Headers
    headers = ['Title', 'URL', 'Repo Owner', 'Repo Name', 'Issue Type', 
               'Difficulty', 'Time Estimate', 'Score']
...
def format_html(issues: List[Dict]) -> str:
    ...
    output.append("<table>")
    output.append("<thead>")
    output.append("<tr><th>#</th><th>Title</th><th>Repo</th><th>Type</th><th>Difficulty</th><th>Score</th><th>URL</th></tr>")
    output.append("</thead>")
    output.append("<tbody>")
...
def format_output(issues: List[Dict], format_type: str, verbose: bool = False, output_file: Optional[str] = None) -> Optional[str]:
    '''
    ...
    if format_type == 'text':
        ...
        raise ValueError(f"Unsupported format: {format_type}. Supported: text, json, table, csv, excel, markdown, html")
