...
class Settings(BaseSettings):
    ...
    database_url: str = Field(
        default="sqlite:///contribution_matcher.db", validation_alias="DATABASE_URL"
    )
    ...
    github_redirect_uri: AnyHttpUrl | None = Field(
        default=None, validation_alias="GITHUB_REDIRECT_URI"
    )
    github_scope: str = Field(default="read:user user:email")
    pat_token: str | None = Field(default=None, validation_alias="PAT_TOKEN")
    ...
    scheduler_discovery_cron: str = Field(
        default="0 6 * * *", validation_alias="SCHEDULER_DISCOVERY_CRON"
    )
    scheduler_scoring_cron: str = Field(
        default="30 6 * * *", validation_alias="SCHEDULER_SCORING_CRON"
    )
    ...
    token_blacklist_cleanup_hours: int = Field(
        default=24, validation_alias="TOKEN_BLACKLIST_CLEANUP_HOURS"
    )
