...
# Difficulty patterns
_BEGINNER_PATTERNS = [
    re.compile(r'\b(beginner|beginner-friendly|good first issue|first timer|starter|easy)\b', re.IGNORECASE),
...
_ADVANCED_PATTERNS = [
    ...
    re.compile(r'\b(requires|needs)\s+(?:deep|extensive|significant)\s+(?:knowledge|experience|understanding)', re.IGNORECASE),
]

# Time estimate patterns
_TIME_PATTERNS = [
    (re.compile(r'should take\s+(?:about\s+)?(\d+)\s*(?:-\s*(\d+))?\s*(?:hour|hr|hours|hrs)', re.IGNORECASE), 'hours'),
    (re.compile(r'takes?\s+(?:about\s+)?(\d+)\s*(?:-\s*(\d+))?\s*(?:hour|hr|hours|hrs)', re.IGNORECASE), 'hours'),
...
# Issue type patterns
_BUG_PATTERNS = [
    re.compile(r'\b(bug|error|issue|problem|broken|fails?|doesn\'?t work|not working)\b', re.IGNORECASE),
...
_DOC_PATTERNS = [re.compile(r'\b(documentation|docs?|readme|guide|tutorial|example)\b', re.IGNORECASE)]
_TEST_PATTERNS = [re.compile(r'\b(test|testing|tests?|coverage|spec|specification)\b', re.IGNORECASE)]
_REFACTOR_PATTERNS = [re.compile(r'\b(refactor|refactoring|cleanup|clean up|restructure|optimize)\b', re.IGNORECASE)]
...
def parse_issue(issue_data: Dict, repo_metadata: Optional[Dict] = None) -> Dict:
    ...
    # Build full repo URL
    full_repo_url = f"https://github.com/{repo_owner}/{repo_name}" if repo_owner and repo_name else None
