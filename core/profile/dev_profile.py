...
def save_dev_profile(profile: Dict, output_path: str = DEV_PROFILE_JSON, encrypt: bool = True) -> None:
    '''
...
def load_dev_profile(json_path: str = DEV_PROFILE_JSON, encrypted: Optional[bool] = None) -> Dict:
    '''
    ...
    if encrypted or json_path.endswith('.encrypted'):
        try:
            ...
            if not service.is_available:
                ...
                if os.path.exists(json_path) and not json_path.endswith('.encrypted'):
                    with open(json_path, 'r', encoding='utf-8') as f:
                        return json.load(f)
                raise ImportError("Encryption service not available and no unencrypted profile found")
            ...
            if os.path.exists(json_path) and not json_path.endswith('.encrypted'):
                with open(json_path, 'r', encoding='utf-8') as f:
                    return json.load(f)
            raise ImportError("cryptography is required for encrypted profiles. Install with: pip install cryptography") from e
        except Exception as e:
            # Fall back to unencrypted file if exists
            if os.path.exists(json_path) and not json_path.endswith('.encrypted'):
                with open(json_path, 'r', encoding='utf-8') as f:
