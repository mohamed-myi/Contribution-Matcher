...
class TestAdvancedFeatureExtraction:
    ...
    def test_get_text_embeddings(self, mock_model, test_db, sample_issue_in_db):
        ...
        assert desc_emb.shape == (384,)  # Original embedding size (PCA happens in extract_advanced_features)
    ...
    def test_embedding_caching(self, test_db, sample_issue_in_db):
        ...
        from core.database import query_issues, get_issue_embedding
...
class TestXGBoostModelTraining:
    ...
    @pytest.mark.skipif(not HAS_XGBOOST or not HAS_SKOPT, reason="XGBoost and scikit-optimize required")
...
class TestModelVersioning:
    ...
    def test_model_version_detection(self, test_db, sample_profile, sample_issue_in_db):
        ...
        from core.database import query_issues, upsert_issue, update_issue_label
        ...
        # Train v2 model
        train_model(force=True, legacy=False, use_advanced=False, use_stacking=False, use_tuning=False)
        ...
        try:
            ...
            # Cleanup
            for f in ["issue_classifier_v2_xgb.pkl", "issue_scaler_v2.pkl", "feature_selector_v2.pkl",
                      "issue_classifier.pkl", "issue_scaler.pkl"]:
                if os.path.exists(f):
                    os.remove(f)
...
class TestThresholdOptimization:
    ...
    @pytest.mark.skipif(not HAS_XGBOOST, reason="XGBoost required")
    def test_threshold_optimization_in_training(self, test_db, labeled_issues_for_ml, sample_profile):
...
class TestHyperparameterOptimization:
    ...
    def test_optimize_hyperparameters(self):
        ...
        best_params = optimize_hyperparameters(X_train, y_train, tune_iterations=10)  # Minimum required
    ...
    @pytest.mark.skipif(not HAS_XGBOOST or not HAS_SKOPT, reason="XGBoost and scikit-optimize required")
    def test_hyperparameter_optimization_in_training(self, test_db, labeled_issues_for_ml, sample_profile):
